...
import Text.Read.Lex qualified as RL
    ...
    showParen (p > consPrec) $
      foldr (.) id $
        intersperse (showString " :~: ") $
          extract (showsPrec (consPrec + 1)) xs
...
class (KnownList es) => Subset (es :: [k]) (es' :: [k]) where
  ...
  reifyIndices =
    unreifiable
      "Subset"
      "Data.Rec.SmallArray.reifyIndices"
